...
session_start();
    ...
    if ( "POST" === $_SERVER['REQUEST_METHOD'] ) {
        ...
        // 3. Définir les contraintes de validation des inputs, et préparer les messages d'erreurs correspondant.
        $formErrors = [];
        ...
        if ( count($formErrors) > 0 ) {
            ...
            // 4a. Sauvegarder les messages d'erreurs préparés en session, pour affichage à l'utilisateur
            $_SESSION['form_errors'] = $formErrors;

            // 4b. Sauvegarder les anciennes données provenant du formulaire en session
            $_SESSION['old'] = $_POST;
            
            // 4b. Effectuer une redirection vers la page de laquelle proviennent les informations,
        ...
        // 7. Efectuer la requête d'insertion du nouveau film en base de données
                    ...
                    <div class="col-md-6 col-lg-4 mx-auto bg-white shadow rounded p-4">
                        ...
                        <form method="post">
                            <div class="mb-3">
                                ...
                                <input type="text" name="title" id="title" class="form-control" value="<?= isset($_SESSION['old']['title']) && !empty($_SESSION['old']['title']) ? htmlspecialchars($_SESSION['old']['title']) : ''; unset($_SESSION['old']['title']); ?>" autofocus required>
                            </div>
                            <div class="mb-3">
